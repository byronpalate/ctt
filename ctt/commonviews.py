...
def panel(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # LISTADO DE ESTUDIANTES Y PROFESORES QUE ESTAN DE CUMPLEAA�OS
        ...
        if 'action' in request.POST:
            ...
            if action == 'cargarfoto':
                try:
                    ...
                    transaction.set_rollback(True)
                    return bad_json(mensaje=u"La imagen seleccionada no cumple los requisitos, de tama�o o formato o hubo un error al guardar fichero.")
...
def cambiocoordinacion(request):
    ...
    if request.method == 'POST':
        try:
            ...
            transaction.set_rollback(True)
            return bad_json(mensaje=u"No se puede cambiar de coordinaci�n.")
    else:
        try:
            data['title'] = u'Cambio de coordinaci�n'
...
def matricular(request, estudiante=False, generarrubros=True):
    ...
    try:
        ...
        if inscripcion.tiene_perdida_carrera():
            return bad_json(mensaje=u"Su l�mite de matricula por perdida de una o mas asignaturas correspondientes a su plan de estudios, ha excedido. Por favor, acercarse a Secretaria para mas informaci�n.")
        ...
        transaction.set_rollback(True)
        return bad_json(mensaje=u"Hubieron errores en la matriculaci�n.")
...
def prematricular(request):
    ...
    try:
        ...
        # PERDIDA DE CARRERA POR 4TA MATRICULA
        if inscripcion.tiene_perdida_carrera():
            return bad_json(mensaje=u"Tiene l�mite de matriculas (4ta matricula).")
        # MATRICULA
        ...
        transaction.set_rollback(True)
        return bad_json(mensaje=u"Hubieron errores en la prematriculaci�n.")
