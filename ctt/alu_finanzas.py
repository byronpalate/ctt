...
def paymentrequest(request, extradata, autorizaciondatafast):
    ...
    clientefacturacion = autorizaciondatafast.inscripcion.clientefacturacion(request)
    ...
    data = {
        ...
        'currency': 'USD',
        'paymentType': 'DB',
        'customer.givenName': u'%s' % remover_caracteres_especiales(autorizaciondatafast.inscripcion.persona.nombres()),
        'customer.middleName': u'%s' % remover_caracteres_especiales(autorizaciondatafast.inscripcion.persona.apellido1),
        'customer.surname': u'%s' % remover_caracteres_especiales(autorizaciondatafast.inscripcion.persona.apellido2),
        'customer.ip': u'%s' % ip,
        'customer.merchantCustomerId': u'%s' % autorizaciondatafast.inscripcion.id,
    ...
    if request.method == 'POST':
        if 'action' in request.POST:
            ...
            if action == 'observaciones':
                try:
                    data = {}
                    data['deposito'] = deposito = DepositoInscripcion.objects.get(pk=int(request.POST['id']))
                    template = get_template("alu_finanzas/observaciones.html")
            ...
            if action == 'act_valor_autorizacion':
                try:
                    ...
                    if not nd:
                        ...
                        identificacion = remover_tildes(inscripcion.persona.identificacion())
                        tipo = inscripcion.persona.tipo_identificacion_comprobante()
                        nombre = remover_tildes(inscripcion.persona.nombre_completo())
                        direccion = remover_tildes(inscripcion.persona.mi_direccion())
                        telefono = remover_tildes(inscripcion.persona.mi_telefono())
                        email = inscripcion.persona.mi_email()
                    if AutorizacionDatafast.objects.filter(inscripcion=inscripcion, activo=True).exists():
                        AutorizacionDatafast.objects.filter(inscripcion=inscripcion, activo=True).update(activo=False)
                    a = AutorizacionDatafast(inscripcion=inscripcion,
                                             autorizacion='',
                                             solicitud='',
                                             respuesta='',
                                             fecha=datetime.now(),
                    ...
                    if autorizacion:
                        if autorizacion['result']['code'] == '000.200.100':
                            ...
                            template = get_template("alu_finanzas/otrosdatos.html")
                            json_content = template.render(data)
            ...
            if action == 'act_valor_autorizacion_payphone':
                try:
                    ...
                    if not nd:
                        ...
                        identificacion = inscripcion.persona.identificacion()
                        tipo = inscripcion.persona.tipo_identificacion_comprobante()
                        nombre = inscripcion.persona.nombre_completo()
                        direccion = inscripcion.persona.mi_direccion()
                        telefono = inscripcion.persona.mi_telefono()
                        email = inscripcion.persona.mi_email()
                    if AutorizacionPayPhone.objects.filter(inscripcion=inscripcion, activo=True).exists():
                        AutorizacionPayPhone.objects.filter(inscripcion=inscripcion, activo=True).update(activo=False)
                    procesador = ProcesadorPagoTarjeta.objects.get(pk=PROVEEDOR_PAGOONLINE_PAYPHONE)
                    a = AutorizacionPayPhone(inscripcion=inscripcion,
                                             autorizacion='',
                                             solicitud='',
                                             respuesta='',
                                             fecha=datetime.now(),
        ...
        if 'action' in request.GET:
            ...
            if action == 'adddeposito':
                try:
                    ...
                    form = DepositoInscripcionForm()
                    data['form'] = form
                    return render(request, "alu_finanzas/adddeposito.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'del':
                try:
                    ...
                    return render(request, "alu_finanzas/del.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'pagar':
                try:
                    ...
                    data['rubros'] = rubros = Rubro.objects.filter(inscripcion=inscripcion,id__in=[int(x) for x in ids]).order_by('fechavence')
                    pp_all_qs = Rubro.objects.filter(inscripcion=inscripcion, validoprontopago=True, cancelado=False, pasivo=False)
                    ...
                    if matricula:
                        precioperiodo = inscripcion.carrera.precio_periodo(inscripcion.mi_nivel().nivel.id, matricula.nivel.periodo, inscripcion.sede, inscripcion.modalidad)
                    ...
                    return render(request, "alu_finanzas/pagar.html", data)
                except Exception as ex:
                    transaction.set_rollback(True)
                    pass
            ...
            if action == 'resultado':
                try:
                    ...
                    return render(request, "alu_finanzas/resultado.html", data)
                except Exception as ex:
                    transaction.set_rollback(True)
                    pass
            ...
            if action == 'resultadopayphone':
                try:
                    ...
                    autorizacion = AutorizacionPayPhone.objects.filter(autorizacion=numeroautorizacion, inscripcion=inscripcion)[0]
                    ...
                    if autorizacion.aprobada:
                        ...
                        try:
                            ...
                            transaction.savepoint_rollback(puntosalva)
                    data['autorizacion'] = autorizacion = AutorizacionPayPhone.objects.filter(autorizacion=numeroautorizacion, inscripcion=inscripcion)[0]
                    return render(request, "alu_finanzas/resultadopayphone.html", data)
                except Exception as ex:
                    transaction.set_rollback(True)
                    pass
            ...
            if action == 'resultadodeuna':
                try:
                    ...
                    # if 'autorizacion' not in request.session:
                    #     return HttpResponseRedirect('/alu_finanzas')
                    return render(request, "alu_finanzas/resultadodeuna.html", data)
                except Exception as ex:
                    transaction.set_rollback(True)
                    pass
